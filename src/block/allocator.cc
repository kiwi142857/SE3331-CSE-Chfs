...
auto BlockAllocator::deallocate(block_id_t block_id) -> ChfsNullResult {
  ...
  // 4. Return KNullOk if everything is fine.

  auto block_idx = block_id % (bm->block_size() * KBitsPerByte);
  auto block_id_ = block_id / (bm->block_size() * KBitsPerByte);

  std::vector<u8> buffer(bm->block_size());
  bm->read_block(block_id_ + this->bitmap_block_id, buffer.data()).unwrap();

  Bitmap bitmap(buffer.data(), bm->block_size());
  if (!bitmap.check(block_idx)) {
    return ChfsNullResult(ErrorType::INVALID_ARG);
  }

  bitmap.clear(block_idx);
  bm->write_block(block_id_ + this->bitmap_block_id, buffer.data()).unwrap();
