...
InodeManager::InodeManager(std::shared_ptr<BlockManager> bm, u64 max_inode_supported) : bm(bm)
{
...
auto InodeManager::create_from_block_manager(std::shared_ptr<BlockManager> bm,
                                             u64 max_inode_supported) -> ChfsResult<InodeManager>
{
    auto inode_bits_per_block = bm->block_size() * KBitsPerByte;
    auto n_bitmap_blocks = max_inode_supported / inode_bits_per_block;

    CHFS_VERIFY(n_bitmap_blocks * inode_bits_per_block == max_inode_supported, "Wrong max_inode_supported");
...
// { Your code here }
auto InodeManager::allocate_inode(InodeType type, block_id_t bid) -> ChfsResult<inode_id_t>
{
    ...
    for (auto iter = iter_res.unwrap(); iter.has_next(); iter.next(bm->block_size()).unwrap(), count++) {
        ...
        if (free_idx) {
            ...
            // 3. Return the id of the allocated inode.
            return ChfsResult<inode_id_t>(RAW_2_LOGIC(inode_index));
        }
    }
...
auto InodeManager::set_table(inode_id_t idx, block_id_t bid) -> ChfsNullResult
{
    ...
    if (idx >= max_inode_supported || bid >= bm->total_blocks() ||
        (bid < n_table_blocks + n_bitmap_blocks && bid != KInvalidBlockID)) {
        return ChfsNullResult(ErrorType::INVALID_ARG);
    }

    // calculate block index and offset
    auto inode_per_block = bm->block_size() / sizeof(block_id_t);
    auto block_index = RAW_2_LOGIC(idx / inode_per_block);
    auto block_offset = idx % inode_per_block;

    // find the block and set the table entry
    bm->write_partial_block(block_index, reinterpret_cast<const u8 *>(&bid), block_offset * sizeof(block_id_t),
                            sizeof(block_id_t));
...
auto InodeManager::get(inode_id_t id) -> ChfsResult<block_id_t>
{
    ...
    // if the parameters are invalid
    if (inode_index >= max_inode_supported) {
        return ChfsResult<block_id_t>(ErrorType::INVALID_ARG);
    }

    // calculate block index and offset
    auto inode_per_block = bm->block_size() / sizeof(block_id_t);
    auto block_index = RAW_2_LOGIC(inode_index / inode_per_block);
    auto block_offset = inode_index % inode_per_block;

    // read the block and get the table entry

    std::vector<u8> buffer(bm->block_size());
    auto read_res = bm->read_block(block_index, buffer.data());

    if (read_res.is_err()) {
        return ChfsResult<block_id_t>(read_res.unwrap_error());
    }

    res_block_id = *reinterpret_cast<block_id_t *>(buffer.data() + block_offset * sizeof(block_id_t));
...
auto InodeManager::get_type_attr(inode_id_t id) -> ChfsResult<std::pair<InodeType, FileAttr>>
{
    std::vector<u8> buffer(bm->block_size());
    auto res = this->read_inode(id, buffer);
    if (res.is_err()) {
    ...
    Inode *inode_p = reinterpret_cast<Inode *>(buffer.data());
    return ChfsResult<std::pair<InodeType, FileAttr>>(std::make_pair(inode_p->type, inode_p->inner_attr));
}

// Note: the buffer must be as large as block size
auto InodeManager::read_inode(inode_id_t id, std::vector<u8> &buffer) -> ChfsResult<block_id_t>
{
...
auto InodeManager::free_inode(inode_id_t id) -> ChfsNullResult
{
    ...
    // simple pre-checks
    if (id >= max_inode_supported - 1) {
        std::cout << "FREE INODE ERROR: " << static_cast<int>(ErrorType::INVALID_ARG) << "   in line: " << __LINE__
                  << std::endl;
        return ChfsNullResult(ErrorType::INVALID_ARG);
    }
    ...
    if (block_id_res.is_err()) {
        std::cout << "FREE INODE ERROR: " << static_cast<int>(block_id_res.unwrap_error()) << "   in line: " << __LINE__
                  << std::endl;
    ...
    if (block_id_res2.is_err()) {
        std::cout << "ERROR: " << static_cast<int>(block_id_res2.unwrap_error()) << "   in line: " << __LINE__
                  << std::endl;
        return ChfsNullResult(block_id_res2.unwrap_error());
    }

    // 2. Clear the inode bitmap.
    inode_id_t bitmap_block_offset = inode_index / (bm->block_size() * KBitsPerByte);
    auto bitmap_bit_offset = inode_index % (bm->block_size() * KBitsPerByte);
    std::vector<u8> buffer(bm->block_size());
