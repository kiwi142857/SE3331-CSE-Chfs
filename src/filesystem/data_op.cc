...
auto FileOperation::write_file(inode_id_t id, const std::vector<u8> &content) -> ChfsNullResult
{
    ...
    if (content.size() > inode_p->max_file_sz_supported()) {
        std::cerr << "file size too large: " << content.size() << " vs. " << inode_p->max_file_sz_supported()
                  << std::endl;
    ...
    if (new_block_num > old_block_num) {
        ...
        if (indirect_block.size() != 0) {
            auto write_res = inode_p->write_indirect_block(this->block_manager_, indirect_block);
            if (write_res.is_err()) {
        ...
        DEBUG_LOG("WRITE INDIRECT BLOCK OK");
        ...
        if (old_block_num > inlined_blocks_num && new_block_num <= inlined_blocks_num && true) {
    ...
    block_id_t block_id = 0;
    {
        ...
        while (write_sz < content.size()) {
            ...
            DEBUG_LOG("WRITE BLOCK OK");
            write_sz += sz;
            block_idx += 1;
        }
    }

    DEBUG_LOG("WRITE FILE OK");
    ...
    while (read_sz < file_sz) {
        ...
        if (inode_p->is_direct_block(read_sz / block_size)) {
            ...
            block_id = reinterpret_cast<block_id_t *>(
                indirect_block.data())[(read_sz / block_size) - inode_p->get_direct_block_num()];
        }
