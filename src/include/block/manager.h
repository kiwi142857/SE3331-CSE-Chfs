...
class BlockManager
{
  ...
  public:
     ...
     */
    virtual auto write_partial_block(block_id_t block_id, const u8 *block_data, usize offset,
     ...
     * @return the block data pointer
     *
     * Note that this function is unsafe because it exposes the internal
     */
     ...
     * @param offset the offset to write the log entry
     * @param data the data to write
     * @param len the length of the data
     *
     * @return Ok(the null result)
     *        Err(INVALID) if the write fails
     */
    auto write_log_entry(usize offset, const u8 *data, usize len) -> ChfsNullResult;

    /**
     * Write a block for recovery
...
class BlockIterator
{
  ...
  public:
    ...
    auto has_next() -> bool
    {
        return this->cur_block_off < (this->end_block_id - this->start_block_id) * bm->block_sz;
    }
