...
TEST_F(RaftTestPart1, LeaderElection)
{
    ...
    TEST_DEBUG_LOG("CheckOneLeader done");

    mssleep(200);
    int term1 = CheckSameTerm();
    TEST_DEBUG_LOG("CheckSameTerm1 done");

    mssleep(200);
    int term2 = CheckSameTerm();
    TEST_DEBUG_LOG("CheckSameTerm2 done");

    EXPECT_EQ(term1, term2) << "inconsistent term";
    TEST_DEBUG_LOG("CheckSameTerm done");
    EXPECT_GE(CheckOneLeader(), 0);
    TEST_DEBUG_LOG("CheckOneLeader done");
}
        ...
        for (int i = 0; i < iters; i++) {
            threads.push_back(std::make_unique<std::thread>(
                [&](int j) {
                    ...
                    bool temp_is_leader;
                    int temp_term, temp_index;
                    auto temp_res = clients[leader]->call(RAFT_RPC_NEW_COMMEND, temp_cmd.serialize(temp_cmd.size()),
                                                          temp_cmd.size());
...
TEST_F(RaftTestPart2, RpcCount)
{
    ...
    for (int tries = 0; tries < 5; tries++) {
        ...
        for (int i = 1; i < iters + 1; i++) {
            ...
            if (res == -1)
                goto loop_end;
            ASSERT_TRUE(res == i) << "wrong value " << res << " committed for index " << index + i << "; expected "
                                  << i;
        }
        ...
        EXPECT_TRUE(total2 - total1 <= rpc_limit)
            << "too many RPCs (" << total2 - total1 << ") for " << iters << " entries" << ", limit is " << rpc_limit;
        success = true;
        break;
    loop_end:
        continue;
