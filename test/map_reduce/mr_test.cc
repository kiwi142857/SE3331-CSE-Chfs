...
class MapReduceTest : public ::testing::Test
{
  protected:
    ...
    const chfs::u16 data_ports[3] = {8081, 8082, 8083};
    ...
    std::vector<std::string> files = {"being_ernest.txt",  "dorian_gray.txt",     "frankenstein.txt",
                                      "metamorphosis.txt", "sherlock_holmes.txt", "tom_sawyer.txt"};
    ...
    void init_fs()
    {
        ...
        meta_srv->run();
        client->reg_server(chfs::ChfsClient::ServerType::METADATA_SERVER, "127.0.0.1", meta_port, true);
        client->reg_server(chfs::ChfsClient::ServerType::DATA_SERVER, "127.0.0.1", data_ports[0], true);
        client->reg_server(chfs::ChfsClient::ServerType::DATA_SERVER, "127.0.0.1", data_ports[1], true);
        client->reg_server(chfs::ChfsClient::ServerType::DATA_SERVER, "127.0.0.1", data_ports[2], true);
    }
